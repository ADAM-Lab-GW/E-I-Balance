...
def writeActivitySection(reportFile, simTag, objFolder, calcDistance=False, epoch=-1):
    ...
    try:
        ...
        excClassActivity, inhClassActivity, excNumSpikes, inhNumSpikes, isiByClass, distance = getActivityISIAndDistanceOverTime(w1_tag, w2_tag, params, objFolder, calcDistance)
    ...
    try:
        ...
        writeTableDistributionHeader(reportFile, htmlClass='dataframe excSpikesNormalized')
        for i in range(params.nb_outputs):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--base_sim_tag', help='The base simTag. Note that \'_#_t#\' will be attached to this string when building the particular simTags for the trials within range')
