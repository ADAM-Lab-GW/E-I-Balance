...
# additional runs may need to be set if the data wasn't complete
settings = [
    # (baseSimTag, baseFileName, fileNum, excRat, w1_std, trial)
    [


for (baseSimTag, baseFileName, dataFolder, fileNum, excRat, w1_std, trial) in settings:
    ...
    f.write('rm '+dataFolder+'/obj/'+simTag+'*.pkl')
    f.write('rm '+dataFolder+'/logs/'+simTag+'*.log')
    f.write('mv '
    f.write(command)
    f.close()
